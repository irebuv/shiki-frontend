...
export const COLOR_THEME_DYNAMIC_ANIMATION =
    'theme-rainbow var(--color-theme-duration) linear infinite';
const COLOR_THEME_PHASE_STORAGE_KEY = 'crack-overlay-start-ms';

export const MIN_DYNAMIC_DURATION_SEC = 10;
export const MAX_DYNAMIC_DURATION_SEC = 1200;

export const DEFAULT_COLOR_THEME_SETTINGS: ColorThemeSettings = {
    mode: 'default',
    manualHue: 237.33,
    dynamicDurationSec: 120,
};
...
function getSyncedDelaySec(durationSec: number): number {
    if (typeof window === 'undefined') return 0;

    const stored = Number(localStorage.getItem(COLOR_THEME_PHASE_STORAGE_KEY));
    const startMs = Number.isFinite(stored) && stored > 0 ? stored : Date.now();

    if (!Number.isFinite(stored) || stored <= 0) {
        localStorage.setItem(COLOR_THEME_PHASE_STORAGE_KEY, String(startMs));
    }
...
export function applyColorThemeSettings(settings: ColorThemeSettings): void {
    ...
    if (safe.mode === 'default') {
        ...
        root.style.setProperty('--color-theme-vars-animation', 'none');
        return;
    }

    //dynamic: changing animation in rainbow way
    if (safe.mode === 'dynamic') {
        const delaySec = getSyncedDelaySec(safe.dynamicDurationSec);
        ...
        root.style.setProperty(
            '--color-theme-vars-animation',
            `color-theme-h-rainbow ${safe.dynamicDurationSec}s linear ${delaySec}s infinite`,
        );

        // Important: avoid double rainbow (desync)
        root.style.setProperty('--color-theme-animation', 'none');
        return;
    }
